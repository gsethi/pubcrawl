...
def getNode(featureid,methMapping):
	featureInfo=featureid.split(":");
	
	if(featureInfo[1].lower()=="gexp"):
	...
	if(featureInfo[1].lower() == "meth"):
		return methMapping[featureInfo[2]]
...
if __name__ == "__main__":
    ...
    methMapping={};
    methFile=open("methMappings.txt","r");
    for line in methFile:
	lineInfo=line.strip().split("\t");
	methMapping[lineInfo[0]]=lineInfo[1].lower();

    methFile.close();
   
    inputFile=open(inputFileString,"r"); 
    outFile=open(fileString+".txt","w");
    outFile.writelines("source\ttarget\tfeatureid1\tfeatureid2\tpvalue\timportance\tcorrelation\tcount\n");
    for line in inputFile:
	rfaceInfo=line.strip().split("\t");      
        sourceNode=getNode(rfaceInfo[0],methMapping);
	targetNode=getNode(rfaceInfo[1],methMapping);
	...
	outFile.writelines(sourceNode + "\t" + targetNode + "\t" + rfaceInfo[0] + "\t" + rfaceInfo[1] + "\t" +  pvalue +  "\t" + rfaceInfo[3] + "\t" + rfaceInfo[4] + "\t" + rfaceInfo[5] +  "\n");
