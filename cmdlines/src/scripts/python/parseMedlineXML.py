...
serverNum = int(sys.argv[2]);
docs_by_server = [[] for i in range(serverNum)]
deleteDocs_by_server = [[] for i in range(serverNum)]
...
for medlineCitation in xmlTag:
	...
	docs_by_server[int(pmid.text) % serverNum].append(doc);
...
for deleteCitation in deleteTag:
	...
	deleteDocs_by_server[int(deleteCitation.text) % serverNum].append(pmid);

for i in range(serverNum):
	docs = docs_by_server[i];
	for j in docs:
		add_element.appendChild(j);
	deletedocs = deleteDocs_by_server[i];
 	for d in deletedocs:
		delete_element.appendChild(d);
	top_element.appendChild(add_element);
	top_element.appendChild(delete_element);
	outFile = codecs.open("core"+str(i)+"/"+sys.argv[3],'w','utf-8');
