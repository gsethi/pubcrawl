...
function loadDeNovoSearches(){
  var dn_query_url = base_query_url + pubcrawl_base_query_uri + pubcrawl_deNovoTerms_query;
//  dn_query.setQuery("select term_value, term_alias,alias");
 var query_str = "select term_value, term_alias,alias";
//'select f1alias, f1values, f2alias, f2values ' + 'where f1alias  = \'' + link.sourceNode.id + '\' and f2alias = \'' + link.targetNode.id + '\' limit 1';
    var query_json = {
        tq: query_str,
        tqx: 'out:json_array'
    };
    var query = '?' + Ext.urlEncode(query_json);

  var dnTerms={searches:null};

  function handleDNTerms(response){
      rows=Ext.decode(response.responseText);
//vq.utils.GoogleDSUtils.dataTableToArray(response.getDataTable());
...
//  dn_query.send(handleDNTerms);
  Ext.Ajax.request({
        url: dn_query_url + query,
        success: handleDNTerms
    });
}
